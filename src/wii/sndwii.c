...
//typedef s16 sndbuf[SOUNDTRUELEN * 2 * 2];
typedef u8 sndbuf[SOUNDTRUELEN * 2 * 2];
sndbuf stereodata16[NUMSOUNDBLOCKS] ATTRIBUTE_ALIGN(32);
#ifdef WIIAUDIO_THREAD
#define AUDIOSTACK (SOUNDTRUELEN * 4 * NUMSOUNDBLOCKS)
...
int SNDWiiInit()
{
   ...
   truesoundoffset = (s16 *)stereodata16[0];
...
#endif
   DCFlushRange((void *)stereodata16[0], SOUNDBUFSIZE);
