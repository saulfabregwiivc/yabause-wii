...
//////////////////////////////////////////////////////////////////////////////
#ifdef WORDS_BIGENDIAN
static INLINE u32 FASTCALL DoColorOffsetBIG(void *info, u32 pixel)
{
...
#ifdef WORDS_BIGENDIAN
      info->PostPixelFetchCalcBIG = &DoColorOffsetBIG;
#endif
   }
   else // color offset disable
      info->PostPixelFetchCalc = &DoNothing;
#ifdef WORDS_BIGENDIAN
      info->PostPixelFetchCalcBIG = &DoNothing;
#endif
...
#else
         if (!TestBothWindow(info->wctl, clip, i << resxratio, j << resyratio))
#endif
         {
            continue;
         }
...
#else
            if (!TestBothWindow(Vdp2Regs->WCTLD >> 8, colorcalcwindow, i << resxratio, j << resyratio))
#endif
               alpha = 0x3F;
            else
...
#ifndef GEKKO
    //iPix = &vdp1backframebuffer[(((y >> vdp1h) / vdp1interlace) * vdp1width) + ((x  >> vdp1w) / 2)];
    iPix = &vdp1backframebuffer[(((y >> vdp1h)) * vdp1width) + ((x  >> vdp1w) / 2)];
#endif
...
#else
            if (!TestBothWindow(wctl, clip, i << resxratio, i2 << resyratio))
#endif
            {
               continue;
            }
...
#else
                        TitanPutPixel(prioritytable[0], i, i2, info.PostPixelFetchCalcBIG(&info, COLSAT2YAB16(0x3F, pixel)), 0);
#endif
               }
               else
               {
...
#else
                        TitanPutPixel(prioritytable[priority], i, i2, info.PostPixelFetchCalcBIG(&info, COLSAT2YAB32(0x20, 0)), 0);
#endif
                     }
                     continue;
                  }
...
#else
                  if (TestBothWindow(Vdp2Regs->WCTLD >> 8, colorcalcwindow, i << resxratio, i2 << resyratio) && (Vdp2Regs->CCCTL & 0x40))
#endif
                  {
...
#else
                  TitanPutPixel(prioritytable[priority], i, i2, info.PostPixelFetchCalcBIG(&info, COLSAT2YAB32(alpha, dot)), 0);
#endif
               }
            }
            else
            {
               // wii version does not use this 8-bit process
               // 8-bit pixel size
