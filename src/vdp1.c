...
#else
   if ((Vdp1Ram = T1MemoryInit(0x160000)) == NULL)
#endif
      return -1;
...
#else
   if ((Vdp1FrameBuffer = T1MemoryInit(0x160000)) == NULL)
#endif
      return -1;
...
u16 FASTCALL Vdp1ReadWord(u32 addr) {
   ...
   switch(addr) {
      ...
      case 0x16:
         return 0x1000 | ((Vdp1Regs->PTMR & 0x2) << 7) | ((Vdp1Regs->FBCR & 0x1E) << 3) | (Vdp1Regs->TVMR & 0xF);
      default:
...
void FASTCALL Vdp1WriteLong(u32 addr, UNUSED u32 val) {
   ...
   switch(addr) {
      case 0x0:
         Vdp1Regs->TVMR = val >> 16;
         Vdp1Regs->FBCR = val & 0xFFFF;
         break;
      case 0x4:
         Vdp1Regs->PTMR = val >> 16;
         Vdp1Regs->EWDR = val & 0xFFFF;
         break;
      case 0x8:
         Vdp1Regs->EWLR = val >> 16;
         Vdp1Regs->EWRR = val & 0xFFFF;
         break;
      case 0xC:
         Vdp1Regs->ENDR = val >> 16;
         break;
      default:
         LOG("trying to long-write a Vdp1 register - %08X\n", addr);
   }
      ...
      if (!(command & 0x4000)) { // if (!skip)
         switch (command & 0x000F) {
            ...
            case 10: // local coordinate
               ...
               break;
#ifdef GEKKO 
// These may be also wrong. But, at leaset, these should not be aborted. 
            case 3: 
               //VIDCore->Vdp1DistortedSpriteDraw();
               break;
            case 7: 
...
#ifdef GEKKO
         case 7: // RETURN, return from subroutine
#endif
            if (returnAddr != 0xFFFFFFFF) {
